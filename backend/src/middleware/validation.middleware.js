...
const validateCreateMeeting = (req, res, next) => {
  ...
  if (req.body.category) {
    validator.isIn(
      'category',
      VALIDATION_RULES.meetingCategory.values,
      VALIDATION_RULES.meetingCategory.message
    );
  }
...
const validatePagination = (req, res, next) => {
  ...
  if (req.query.status) {
    validator.isIn(
      'status',
      VALIDATION_RULES.meetingStatus.values,
      VALIDATION_RULES.meetingStatus.message
    );
  }

  if (!validator.isValid()) {
    return next(
      new AppError('Invalid query parameters', 400, 'VALIDATION_ERROR').setDetails(
        validator.getErrors()
      )
    );
  }
...
const validateSearch = (req, res, next) => {
  ...
  if (!q) {
    return next(new AppError('Search query (q) is required', 400, 'VALIDATION_ERROR'));
  }

  if (q.trim().length < 2) {
    return next(
      new AppError('Search query must be at least 2 characters', 400, 'VALIDATION_ERROR')
    );
  }
...
const validateUUIDParam = (paramName = 'id') => {
  return (req, res, next) => {
    ...
    if (!id) {
      return next(new AppError(`${paramName} is required`, 400, 'VALIDATION_ERROR'));
    }
...
const validateAccountDeletion = (req, res, next) => {
  ...
  if (!confirmation) {
    return next(new AppError('Confirmation is required', 400, 'VALIDATION_ERROR'));
  }
...
const validateDownloadFormat = (req, res, next) => {
  ...
  if (format && !['txt', 'json'].includes(format)) {
    return next(new AppError('Invalid format. Must be "txt" or "json"', 400, 'INVALID_FORMAT'));
  }
